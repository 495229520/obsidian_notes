...
struct PeerInfo {
    std::string peer_id;
    sockaddr_in observed_addr{}; // 服务器通过 recvfrom() 实际观察到的公网地址
};
...
static void send_text(int sock, const sockaddr_in& to, const std::string& s) {
    // 学习版里直接发纯文本协议，真实系统通常还会带消息类型、长度、校验等字段。
    sendto(sock, s.data(), s.size(), 0,
           reinterpret_cast<const sockaddr*>(&to), sizeof(to));
}

int main() {
    // 引介服务器本身也是一个普通 UDP 服务端：
    // 只不过它不转发业务流量，只负责“登记 peer 并交换公网端点”。
    ...
    bind_addr.sin_port = htons(40000); // 固定监听端口，供所有 peer 注册
    ...
    // room -> peers：
    // 学习版里假设每个房间只容纳 2 个 peer，便于演示 A/B 打洞。
    ...
    while (true) {
        ...
        for (auto& p : peers) {
            if (p.peer_id == peer_id) {
                // 同一个 peer 重复注册时，刷新它当前被 NAT 映射出来的公网端点
        ...
        if (!updated) {
            // 第一次看到这个 peer，就把“服务器实际观察到的公网地址”记下来
            peers.push_back(PeerInfo{peer_id, from});
        }

        if (peers.size() == 2) {
            // 房间里两个人都到了，开始互相介绍：
            // 把 A 看到的 B 公网地址发给 A，把 B 看到的 A 公网地址发给 B。
...
int main() {
    // 这些参数在真实系统里通常来自配置、信令层或者 UI 输入。
    std::string server_ip = "203.0.113.10";
    uint16_t server_port = 40000;
    std::string room = "room1";
    std::string my_id = "A";
    uint16_t local_port = 50000; // 明确绑定本地端口，方便演示“同一个 socket / 同一个本地端点”
    ...
    // 第一步：先联系引介服务器注册自己。
    // 注意：注册使用的就是后面打洞和通信要继续复用的同一个 UDP socket。
    send_text(sock, server, "REG " + room + " " + my_id);

    sockaddr_in peer_addr{};          // 当前认定的对端地址
    std::atomic<bool> connected{false}; // 是否已经收到对端真实回包，确认洞已打通
    char buf[2048];
    std::thread punch_thread;         // 后台持续发 PUNCH，避免“只打一包就停”
    ...
    while (true) {
        ...
        if (cmd == "PEER") {
            ...
            // 这是引介服务器告诉我们的“对端当前公网端点”。
            // 这个地址只是“开始尝试打洞的目标”，后面真正打通后仍要以实际收到的源地址为准。
            peer_addr = {};
            ...
            if (recv_room == room) {
                // 对端已经从它的同一个 socket 主动打过来了：
                // 这说明双方 NAT 很可能都已经为彼此放行，可以立刻回 ACK。
                send_text(sock, from, "PUNCH_ACK " + room + " " + my_id);
                connected = true;
                peer_addr = from; // 以实际收到的源地址为准，避免信令地址与真实可达地址不一致
            }
        }
            ...
            if (recv_room == room) {
                connected = true;
                peer_addr = from; // 收到 ACK 也说明这条 from 地址已经真实可达
            }
        }
        else if (cmd == "DATA") {
            std::string peer_id;
