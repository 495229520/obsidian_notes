...
- 不讲通用 TCP/IP 入门，前置知识可回看 [[3.1 套接字]]。
...
![[图片/6.项目/04.Linux播放器服务器/4.2 网络/4_2_1_1.svg|980]]
...
这里的 Reactor 不是某个系统调用，而是一种组织方式：`epoll` 只告诉我们“哪个 fd 就绪”，Reactor 负责把就绪事件分发到正确的对象和回调。这个分层可以结合 [[6.6 Reactor模式与EventLoop]] 理解。
...
- backlog、半连接队列、全连接队列的压力可以结合 [[8.2 TCP内核队列与参数调优]] 理解。
...
- 连接对象的生命周期必须和事件回调绑定清楚，回调触发时不能访问已经释放的对象；这部分可以继续看 [[6.8 连接对象生命周期与RAII]]。
...
- 非阻塞 fd 读到 `EAGAIN` 不是错误，而是“当前已经读完”。非阻塞收发细节见 [[6.10 非阻塞connect与健壮收发]]，粘包拆包见 [[4.7 TCP的粘包和拆包]]。
...
- input/output buffer 的必要性可以结合 [[4.6 IO缓冲]] 理解。
...
- 这个模型可以和 [[6.7 eventfd、timerfd与跨线程唤醒]] 一起理解；线程池侧则复用 [[4.1.5 CThread 与 CThreadPool 实现]] 的任务执行能力。
...
- 连接超时与心跳设计可以继续看 [[6.9 定时器、心跳与连接超时]]。
...
| 风险 | 典型后果 | 处理思路 |
|---|---|---|
| ET 模式只读一次 | 缓冲区残留数据，后续不再触发 | 非阻塞循环读到 `EAGAIN`，见 [[6.4 epoll模型]] |
...
| 回调中释放连接 | 后续事件访问悬挂对象 | 统一关闭路径，延迟回收，参考 [[6.8 连接对象生命周期与RAII]] |
| 业务线程直接操作连接 | 数据竞争、关闭顺序混乱 | 通过 `queueInLoop()` 回投 I/O 线程 |
| 超时关闭和正常关闭竞争 | 重复 close、重复删除事件 | 用连接状态机合并关闭路径 |
| accept 慢或 backlog 太小 | 新连接建立失败、排队溢出 | 结合 [[8.2 TCP内核队列与参数调优]] 检查 |
...
- [[6.6 Reactor模式与EventLoop]]：网络模板的主干架构，解释 epoll 之后为什么还需要事件循环和回调分发。
- [[6.4 epoll模型]]：I/O 复用底层基础，特别是 LT/ET、`EPOLLOUT`、错误事件和关闭顺序。
- [[6.7 eventfd、timerfd与跨线程唤醒]]：支撑跨线程任务回投和时间事件纳入事件循环。
- [[6.8 连接对象生命周期与RAII]]：支撑连接对象、fd 所有权、回调安全和关闭顺序。
- [[6.9 定时器、心跳与连接超时]]：支撑心跳、空闲连接清理和超时关闭。
- [[6.10 非阻塞connect与健壮收发]]：支撑非阻塞读写、短读短写和输出缓冲策略。
- [[4.7 TCP的粘包和拆包]]：支撑 input buffer 与协议解析边界。
- [[4.6 IO缓冲]]：支撑内核缓冲区、应用层缓冲区和短读短写理解。
- [[2.5 TCP连接关闭与异常状态]]：支撑正常关闭、RST、半关闭和异常路径。
- [[3.2 套接字选项]]：支撑 keepalive、地址复用等 socket 参数设置。
- [[3.1 套接字]]、[[3.3 listen和accept函数]]：作为基础 API 回顾。
