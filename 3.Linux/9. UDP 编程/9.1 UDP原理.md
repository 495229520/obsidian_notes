...
UDP，全称 **User Datagram Protocol**，中文是**用户数据报协议**。它和 [[7.6 TCP协议基础|TCP]] 一样，都是互联网 [[7.5 传输层协议：TCP与UDP|协议]] 栈里的传输层协议，但设计思路完全不同。

UDP 的核心风格可以概括成一句话：

**快、轻量、面向报文，但不保证可靠。**

它不会像 TCP 那样先做 [[7.6 TCP协议基础#4. 三次握手|三次握手]]，而是直接把一个个数据报发出去。也正因为这样，UDP 非常适合实时性高、允许少量丢包的场景。
...
| 步骤 | 说明 |
|:---:|:---|
| **打包** | 你把一段应用数据装进一个数据报里，并写上目标 [[7.2 IP地址与CIDR]] 和 [[7.3 端口与五元组]] |
...
- **原理**：发送前不需要像 [[7.6 TCP协议基础#4. 三次握手|TCP 那样握手建连]]。
...
服务器需要 [[7.7 套接字1#4. bind 函数|bind]] 的核心原因是：
