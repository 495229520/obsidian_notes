...
- 这是一个套接字描述符，用于标识 UDP 服务端的 [[7.7 套接字1|套接字]]
- 通过 `socket()` 函数创建
...
- 第二个参数使用 `SOCK_DGRAM` 表示创建 UDP 类型的 [[7.7 套接字1#2.1 domain 与 type 的组合|套接字]]
...
3. **recvfrom 获取客户端地址**：通过 `recvfrom()` 的输出参数获取发送方 [[7.2 IP地址与CIDR|地址]] 和 [[7.3 端口与五元组|端口]]
