...
- **filename**：新文件的路径
- **mode**：打开模式（如 `"r"`、`"w"`、`"a"` 等），与 [[6.1 基本文件函数#文件打开模式|fopen 的模式]]相同
...
> `fflush()` 只完成一半的路程：把 stdio 用户态缓冲里的数据通过 `write()` 交给内核页缓存。此时断电数据照样会丢。要保证写到磁盘介质，还需要第二步 [[4.2 重定向、同步#fsync() - 文件同步|fsync()]]：先 `fflush(fp)`，再 `fsync(fileno(fp))`。`fclose()` 同理：它会自动 fflush，但不会 fsync。
>
