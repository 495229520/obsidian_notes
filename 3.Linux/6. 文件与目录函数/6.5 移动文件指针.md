...
- `fseek()` 是 [[6.3 读取文件#文件内部指针|"+" 模式读写切换规则]]认可的"定位函数"：读转写、写转读之间插一次 `fseek()` 即合法，哪怕位置没变（`fseek(fp, 0, SEEK_CUR)`）
...
> - 想知道一个文件哪里是空洞，用 [[4.2 重定向、同步#lseek() - 修改读写位置偏移量|lseek()]] 的 `SEEK_HOLE` / `SEEK_DATA`（Linux 3.1 起支持）

### fseek 与 lseek 的对比

`fseek()` 是 C 标准库函数（操作 `FILE*` 流），而 [[4.2 重定向、同步#lseek() - 修改读写位置偏移量|lseek()]] 是 POSIX 系统调用（操作[[4.1 打开、读取、写入、关闭#文件描述符（File Descriptor）|文件描述符]]）。
...
"不可定位"这件事是内核那一层的性质，所以换成 `lseek()` 也绕不过去，详见[[#不可 seek 的流：管道、套接字、终端|后面一节]]。
...
三个前提：文件要以二进制模式打开（文本模式下 `ftell()` 的返回值只保证"能传回 `fseek()`"，不保证等于字节数）；文件大小不能超过 `long` 的表示范围，超了就用下面的 `ftello()`；流本身得能定位，管道和套接字上这套写法会失败，见[[#不可 seek 的流：管道、套接字、终端|后面一节]]。
...
## 不可 seek 的流：管道、套接字、终端

上面几个函数都默认"文件有一个可以任意跳转的位置"。这个前提对普通文件成立，对[[11.3 管道通讯|管道]]、FIFO、套接字和终端都不成立：这些流只能顺着往下读，没有"第 100 个字节"这种概念。在它们上面调用定位函数会失败，`errno` 置为 `ESPIPE`（`Illegal seek`）：
...
与其 `fseek` + `ftell` 一来一回，不如直接问内核。[[4.1 打开、读取、写入、关闭#文件描述符（File Descriptor）|文件描述符]]上的 `fstat()` 不移动位置，也不受 `long` 宽度限制：
