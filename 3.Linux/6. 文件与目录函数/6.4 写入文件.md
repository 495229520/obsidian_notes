...
`fwrite()` 和 [[6.3 读取文件#fread() - 二进制读取|fread()]] 是一对：用 `fwrite()` 把结构体按内存字节原样写入文件，再用相同的 `size`/`count` 参数调用 `fread()` 就能把它读回内存。
...
更多关于缓冲区分层和 `fflush()` 的内容，参见[[6.2 高级文件操作函数#fflush() - 刷新缓冲区|高级文件操作函数中的 fflush 介绍]]。
