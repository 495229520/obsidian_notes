...
> 它不改变字节流的内容，也不改变接收端 `read()` 一次能读到多少。**关掉 Nagle 不等于解决粘包**，粘包本来就不是发送端能控制的事，见 [[4.7 TCP的粘包和拆包]]。
...
- **返回值要检查。** `writev()` 和 `write()` 一样可能短写，非阻塞套接字上尤其常见。真实代码要把没发完的部分留在应用层缓冲区里，等可写事件再续，见 [[6.6 Reactor模式与EventLoop]]。

`writev` 的完整用法和零拷贝的其他手段见 [[8.3 高性能发送与线上排障#6. writev 和 readv|writev 和 readv]]。
...
复制被删掉是必须的：`CorkGuard` 管的是「解除」这个动作，允许复制就会解除两次，第二次作用在一个可能已经关闭的 fd 上。这和 [[6.8 连接对象生命周期与RAII|连接对象的 RAII]] 是同一套思路。
...
**它和流控制、拥塞控制是三道独立的闸。** 滑动窗口管「对方还收不收得下」，拥塞窗口管「网络还塞不塞得进」，Nagle 管「这么点数据值不值得现在占一个包」。三道都放行才会真的发出去，见 [[2.3 流控制#2.5 糊涂窗口综合症|糊涂窗口综合症]]。

**和糊涂窗口综合症是一个问题的两端。** 接收端那侧的对策是不通告小窗口，发送端这侧的对策就是 Nagle。

**和粘包无关。** 关掉 Nagle 之后每次 `write()` 确实更可能对应一个报文，但接收端什么时候来读、一次读走多少，依然不受发送方控制。消息边界只能靠应用层协议定，见 [[4.7 TCP的粘包和拆包]]。
...
- [[3.2 套接字选项]] - `setsockopt`/`getsockopt` 的用法与其他选项
- [[2.6 TIME_WAIT状态]] - 同一章的另一个默认行为陷阱
- [[8.3 高性能发送与线上排障]] - `writev`、`sendfile`、`TCP_CORK` 与三层排障
- [[4.7 TCP的粘包和拆包]] - 为什么关掉 Nagle 也解决不了消息边界
- [[2.3 流控制]] - 滑动窗口、糊涂窗口综合症与 Nagle 的分工
- [[4.6 IO缓冲]] - 发送缓冲区、攒包与延迟的取舍
