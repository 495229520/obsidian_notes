...
UDP 服务器是一种无连接的服务器架构，与 [[4.1 TCP服务端|TCP 服务端]] 相比更加简洁。由于 [[5.1 UDP原理|UDP 协议]] 的无连接特性，服务器不需要调用 `listen()` 和 `accept()` 函数来监听和接受连接。
...
![[图片/3.Linux/9. UDP 编程/Pasted image 20251218085248.png]]
---
...
- 这是一个套接字描述符，用于标识 UDP 服务端的 [[3.1 套接字|套接字]]
- 通过 `socket()` 函数创建

### 2.2 `void *buff_snd` / `ssize_t snd_len` / `int flags`
...
```mermaid
flowchart TD
    A["socket() 创建套接字"] --> B["bind() 绑定地址和端口"]
    B --> C["recvfrom() 接收数据（阻塞等待）"]
    C --> D["处理数据"]
    D --> E["sendto() 发送响应（可选）"]
    E --> F["close() 关闭套接字"]
```
...
- 第二个参数使用 `SOCK_DGRAM` 表示创建 UDP 类型的 [[3.1 套接字#2.1 domain 与 type 的组合|套接字]]
...
3. **recvfrom 获取客户端地址**：通过 `recvfrom()` 的输出参数获取发送方 [[1.2 IP地址与CIDR|地址]] 和 [[1.3 端口与五元组|端口]]
4. **sendto 指定目标地址**：每次发送都需要指定目标地址
5. **配套客户端**：参见 [[5.3 UDP客户端代码|UDP 客户端代码]]
