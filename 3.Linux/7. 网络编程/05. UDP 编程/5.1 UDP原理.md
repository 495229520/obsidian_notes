...
UDP，全称 **User Datagram Protocol**，中文是**用户数据报协议**。它和 [[2.1 TCP协议基础|TCP]] 一样，都是互联网 [[1.5 传输层协议：TCP与UDP|协议]] 栈里的传输层协议，但设计思路完全不同。

UDP 的核心风格可以概括成一句话：

**快、轻量、面向报文，但不保证可靠。**

它不会像 TCP 那样先做 [[2.1 TCP协议基础#4. 三次握手|三次握手]]，而是直接把一个个数据报发出去。也正因为这样，UDP 非常适合实时性高、允许少量丢包的场景。

![[图片/3.Linux/9. UDP 编程/ec04b363-0c7d-445e-8021-10627573d6fe.png]]
...
| 步骤 | 说明 |
|:---:|:---|
| **打包** | 你把一段应用数据装进一个数据报里，并写上目标 [[1.2 IP地址与CIDR]] 和 [[1.3 端口与五元组]] |
...
![[图片/3.Linux/9. UDP 编程/9_1_6.svg|1080]]
...
![[图片/3.Linux/9. UDP 编程/9_1_7.svg|1020]]
...
### 5.1 无连接（Connectionless）

- **原理**：发送前不需要像 [[2.1 TCP协议基础#4. 三次握手|TCP 那样握手建连]]。
...
服务器需要 [[3.1 套接字#4. bind 函数|bind]] 的核心原因是：
...
![[图片/3.Linux/9. UDP 编程/9_1_8.svg|1160]]
...
如果你想继续看拆分实现，可以接着读 [[5.2 UDP服务端代码|UDP 服务端代码]] 和 [[5.3 UDP客户端代码|UDP 客户端代码]]。

---

## 10. 工程上最容易踩的坑

### 10.1 把 UDP 当成“简化版 TCP”
...
UDP 支持广播和组播，这是 TCP 没有的典型能力。

| 类型 | 说明 | 典型场景 |
|:---|:---|:---|
| 广播 | 发给同一广播域内所有主机 | 局域网发现 |
| 组播 | 发给加入同一组播组的主机 | 视频分发、行情推送 |

广播发送前通常要设置 `SO_BROADCAST`：
...
组播则涉及加入组播组、选择网卡、TTL 等选项，工程上还要考虑交换机、路由器和防火墙是否支持。

### 11.2 MTU、分片与 PMTU

UDP 保留消息边界，但不保证大报文可靠到达。一个 UDP 数据报如果超过路径 MTU，IP 层可能分片；任意一个分片丢失，整个 UDP 数据报都无法还原。

工程经验：

- 互联网场景尽量控制单个 UDP payload，不要贴近 64KB 理论上限
- 实时音视频、游戏、QUIC 都会主动控制包大小
- 需要大数据传输时，应用层自己分片、编号、重组、重传
...
这也是为什么“用 UDP 自己实现可靠传输”并不简单，它会逐渐长出一套类似传输层的机制。

### 11.4 UDP 与 QUIC

QUIC 基于 UDP，但不是“裸 UDP”。它在用户态实现了很多 TCP/TLS/HTTP2 的能力：
...
面试回答可以这样说：UDP 本身不可靠，QUIC 是在 UDP 之上重新设计了一套可靠、安全、支持多路复用的传输协议。
...
9. **UDP 大包要警惕 MTU 和 IP 分片**：分片丢一片，整个数据报都失败。
10. **QUIC 是基于 UDP 的高级传输协议**，不是 UDP 自带可靠性。
