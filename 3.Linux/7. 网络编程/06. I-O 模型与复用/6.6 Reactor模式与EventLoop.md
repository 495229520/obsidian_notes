...
学完 [[6.4 epoll模型|epoll]] 之后，已经知道 Linux 可以用一个线程高效等待大量 fd 的就绪事件。但 `epoll_wait()` 只回答一个问题：
...
可以把 [[6.1 IO复用简介|I/O 复用]] 和 Reactor 的关系理解成：
...
- `epoll_event.data.ptr` 在真实 C++ 网络库中常指向 `Channel` 或连接相关对象，因此生命周期必须清晰，后面会在 [[6.8 连接对象生命周期与RAII]] 继续展开。
...
其中 `Channel::handleEvent()` 背后的回调注册与触发机制，可以继续看 [[6.6.1 回调函数与Reactor事件分发]]。

> [!warning] ET 模式不要只读一次
> 如果底层使用 `EPOLLET`，可读事件到来后必须配合非阻塞 fd 循环 `read()` / `recv()` 到 `EAGAIN`。否则内核缓冲区中残留的数据可能不会再次触发通知。
>
> 非阻塞收发细节见 [[6.10 非阻塞connect与健壮收发]]。
...
跨线程投递的完整链路（`eventfd` 怎么写、怎么读）在 [[6.7 eventfd、timerfd与跨线程唤醒]]，这里只关心 EventLoop 这一侧。
...
跨线程投递后，如果 EventLoop 正阻塞在 `epoll_wait()`，就需要借助 `eventfd` 等机制唤醒它，这部分可以继续看 [[6.7 eventfd、timerfd与跨线程唤醒]]。
...
TCP 是字节流协议，不保留应用层消息边界。因此 read handler 只是把字节读入缓冲区，真正的“消息”需要由协议解析层从 input buffer 中拆出来。粘包和拆包问题见 [[4.7 TCP的粘包和拆包]]。
...
所以 `EPOLLOUT` 不是默认常驻事件，而是“有待发送数据且暂时发不完”时才打开。完整的健壮发送逻辑可以和 [[6.10 非阻塞connect与健壮收发]] 一起理解。
...
TCP 关闭状态与异常情况见 [[2.5 TCP连接关闭与异常状态]]，连接对象的 RAII 管理见 [[6.8 连接对象生命周期与RAII]]。
...
短连接密集、accept 本身成为瓶颈时（比如压测里每秒几万次连接建立），`SO_REUSEPORT` 的收益很直接。长连接为主的服务通常无所谓，accept 一天也跑不了几次。选项细节和惊群问题见 [[8.2 TCP内核队列与参数调优]]。
