...
`epoll` 是 Linux 特有的 [[6.1 IO复用简介|I/O 复用]] 机制，从 Linux 2.6 内核开始引入。相比 [[6.2 select模型|select]] 和 [[6.3 poll模型|poll]]，epoll 在处理大量并发连接时具有显著的性能优势，是 **Linux 高性能网络编程的首选方案**。
...
这张图要说的是：**全程没有任何一步在遍历监控集合。** 是「谁有事谁自己举手」，不是「挨个问一遍谁有事」。所以 `epoll_wait()` 的成本跟就绪个数走，跟总连接数无关。它比 [[6.2 select模型|select]]、[[6.3 poll模型|poll]] 适合高并发，原因就在这里，不在「用了红黑树」——红黑树只加速 `epoll_ctl()` 的增删改。
...
对照 [[6.2 select模型|select]] 看更清楚：select 把「注册」和「等待」压在同一个系统调用里，所以每轮都要重来一遍；epoll 把它们拆成了 `ctl` 和 `wait` 两个调用，注册的成本才有机会被摊掉。
...
epoll 生态里更常见的做法是 `signalfd`：把信号变成一个可读的 fd 丢进 epoll，和普通连接事件走同一条处理路径，就不用再纠结信号处理函数里哪些函数可以调。定时器（`timerfd`）和跨线程唤醒（`eventfd`）用的是同一个思路，见 [[6.7 eventfd、timerfd与跨线程唤醒]]。
...
sequenceDiagram
    autonumber
    participant EvLoop as 事件循环线程
    participant Buf as 内核接收缓冲区
    participant Other as 其他连接

    Buf->>EvLoop: 数据到达，ET 触发一次
    EvLoop->>Buf: read() 读到数据
    EvLoop->>Buf: read() 又读到数据
    Note over Buf: 缓冲区已被读空
    EvLoop->>Buf: 再 read() 一次
    activate EvLoop
    Note over EvLoop,Buf: 阻塞 fd：不返回 EAGAIN，线程直接睡在这里
...
sequenceDiagram
    autonumber
    participant EP as epoll 实例
    participant EvLoop as 事件线程
    participant W as worker 线程
    participant B as 其他 worker

    EP-->>EvLoop: fd=10 可读（带 EPOLLONESHOT）
    Note over EP: 该 fd 就此失效，不再产生事件
    EvLoop->>W: 把 fd=10 交给线程池
    W->>W: read、解析、更新连接状态
    EP--x B: 这期间不会把 fd=10 派给别的 worker
    W->>EP: epoll_ctl(MOD) 重新注册 EPOLLONESHOT
    EP-->>EvLoop: 之后才会再次报告 fd=10
```
...
> 它只表示内核发送缓冲区能再接收一些字节，跟对端有没有收到、什么时候收到没有任何关系。数据什么时候真正上路，还要看发送端的 Nagle 判定和窗口，那是 [[8.1 Nagle算法]] 的内容，这里不重复。

这和 [[6.10 非阻塞connect与健壮收发#5. 健壮发送：处理短写|非阻塞短写处理]] 是同一套工程模型。
...
- [[6.1 IO复用简介]] - I/O 模型与 I/O 复用定位
- [[6.2 select模型]] - select 的位图、限制和线性扫描
- [[6.3 poll模型]] - pollfd 数组与 poll 模型
- [[6.6 Reactor模式与EventLoop]] - 事件循环与 Channel 的落地形态
- [[6.7 eventfd、timerfd与跨线程唤醒]] - 把定时器和信号也变成 fd 丢进 epoll
- [[6.10 非阻塞connect与健壮收发]] - 非阻塞读写、短写和反压
- [[2.5 TCP连接关闭与异常状态]] - FIN、RST、CLOSE_WAIT 和关闭策略
- [[8.2 TCP内核队列与参数调优]] - backlog、队列和 accept 压力
