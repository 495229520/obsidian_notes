...
阻塞 socket 适合教学，但真实服务通常使用非阻塞 socket 配合 [[3.Linux/7. 网络编程/06. I-O 模型与复用/6.4 epoll模型|epoll]]。非阻塞编程的核心不是“系统调用不阻塞”这么简单，而是要正确处理：
...
这也是 [[6.4 epoll模型#8.2 边缘触发（Edge Triggered, ET）|epoll ET]] 必须配合非阻塞循环读写的原因。
...
要注意反压最终会一路传导回对端：本端不读 → 接收缓冲区满 → `rwnd` 降到 0 → 对端 `send()` 被卡住。整条链路的机制见 [[2.3 流控制]]。这是好事——它把“我处理不动了”如实地告诉了上游，而不是把请求堆在内存里直到 OOM。
...
- [[2.5 TCP连接关闭与异常状态]] - 关闭、RST、SIGPIPE、CLOSE_WAIT
- [[4.7 TCP的粘包和拆包]] - 输入缓冲区与应用层拆包状态机
- [[4.6 IO缓冲]] - TCP 发送缓冲区与接收缓冲区
- [[2.3 流控制]] - 反压最终怎么传导回对端
- [[6.4 epoll模型]] - epoll 工程陷阱与 ONESHOT
