...
`poll()` 是 [[6.1 IO复用简介|I/O 复用]] 的另一种实现，它是对 [[6.2 select模型|select]] 的改进，解决了 select 的一些限制问题。`poll()` 源自 System V Unix，在 POSIX 标准中也有定义。
...
实际写法是：先把 `POLLIN` 里的数据读干净，再处理关闭。反过来先关连接，缓冲区里的尾包就丢了。详见 [[2.5 TCP连接关闭与异常状态]]。
...
> 因为监控集合根本不在应用手里。`epoll_ctl(DEL)` 之后内核自己把 epitem 从红黑树摘掉，没有数组、没有空洞、也没有 `nfds`。见 [[6.4 epoll模型#2. epoll 的三个核心对象]]。
...
| `write()` 返回值没检查 | 短写丢数据；对端已关时 `SIGPIPE` 杀进程 | 处理短写 + 忽略 `SIGPIPE`，见 [[6.10 非阻塞connect与健壮收发#5. 健壮发送：处理短写|健壮发送]] |
...
- [[6.2 select模型]] - 位图、值-结果参数与三段 O(n)
- [[6.4 epoll模型]] - 监控集合留在内核，拷贝和线性扫描一起消失
- [[2.5 TCP连接关闭与异常状态]] - FIN、RST 与半关闭的处理顺序
