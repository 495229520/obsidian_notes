...
[[6.4 epoll模型|epoll]] 只回答一件事：哪些 fd 现在可读可写。它不会主动告诉应用：
...
这些都是时间事件。成熟的 [[6.6 Reactor模式与EventLoop|EventLoop]] 同时管着 socket fd、`eventfd`、`timerfd` 和一个 timer queue。
...
| 机制 | 层次 | 判断什么 | 时间尺度 |
|:---|:---|:---|:---|
| TCP keepalive | 内核 TCP 层 | 这条 TCP 连接在网络上还通吗 | 默认 2 小时起，见 [[3.2 套接字选项]] |
...
`Entry` 里存 `weak_ptr<TcpConnection>` 而不是 `shared_ptr`，是为了避免时间轮反过来延长连接的寿命——连接的生命周期归连接表管，时间轮只是观察者。这和 [[6.8 连接对象生命周期与RAII|连接生命周期]] 里 `Channel::tie_` 的选择是同一个理由。
...
定时器判定超时之后，关闭动作还是得按 [[6.8 连接对象生命周期与RAII|连接对象生命周期]] 的规则来：
...
- [[6.4 epoll模型]] - epoll 只负责 fd 就绪，不负责业务超时
- [[6.6 Reactor模式与EventLoop]] - EventLoop 中 timer queue 的位置
- [[6.7 eventfd、timerfd与跨线程唤醒]] - `timerfd` 的系统调用语义和 epoll 集成
- [[6.8 连接对象生命周期与RAII]] - 超时关闭时的安全释放顺序与 `weak_ptr` 观察关系
- [[2.5 TCP连接关闭与异常状态]] - TCP 关闭、半关闭、RST 与异常状态
- [[3.2 套接字选项]] - TCP keepalive 等 socket 选项
