...
这和 [[6.6 Reactor模式与EventLoop|Reactor]] 的角色划分一致：`Channel` 是事件处理入口，`TcpConnection` 是连接状态和资源的聚合点。
...
> 前提是这个 fd 的**所有**副本都关了。只要还有 `dup()` 出来的副本、或者 fork 后子进程持有的副本活着，内核里的 file description 就没销毁，epoll 的注册项也就还在。详见 [[6.4 epoll模型#15. 关闭连接的顺序|epoll 的关闭连接顺序]]。

TCP 层面的关闭状态（`FIN`、半关闭、`RST`）见 [[2.5 TCP连接关闭与异常状态]]。
...
投递机制见 [[6.7 eventfd、timerfd与跨线程唤醒|跨线程唤醒]]。
...
- [[6.4 epoll模型]] - epoll 关闭顺序与 `data.ptr` 生命周期风险
- [[6.6 Reactor模式与EventLoop]] - Channel、TcpConnection、EventLoop 的职责划分
- [[6.7 eventfd、timerfd与跨线程唤醒]] - 跨线程投递任务回所属 EventLoop
- [[6.9 定时器、心跳与连接超时]] - 定时器回调里的 `weak_ptr` 与超时关闭
- [[2.5 TCP连接关闭与异常状态]] - TCP 关闭状态与异常连接处理
