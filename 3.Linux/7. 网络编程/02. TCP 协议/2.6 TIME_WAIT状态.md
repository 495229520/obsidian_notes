...
[[2.1 TCP协议基础#5. 四次挥手|四次挥手]]结束时，两端的结局并不对称：**主动调 `close()` 的那一方**在发出最后一个 ACK 后进入 `TIME_WAIT`，被动方收到这个 ACK 就直接 `CLOSED`。
...
`CLOSE_WAIT` 堆积和 `TIME_WAIT` 堆积是完全相反的两回事，别混：前者在被动关闭方、是应用忘了 `close()` 的 bug，后者在主动关闭方、是协议要求的正常现象。详见 [[2.5 TCP连接关闭与异常状态#2. 先看全景：两个状态各落在哪一端|两个状态各落在哪一端]]。
...
它**不**做的事同样重要：它不会让两个套接字同时 `listen` 在一个 `IP:port` 上，那是 `SO_REUSEPORT` 的活。两者的区别见 [[3.2 套接字选项#6.4 SO_REUSEADDR 与 SO_REUSEPORT|套接字选项]]。
...
- **`listen()` 的 backlog 别写 3。** 用 `SOMAXCONN` 或按并发量估，写太小在压力下会丢连接，见 [[8.2 TCP内核队列与参数调优|内核队列]]。
...
- **`SO_LINGER` 设成 0**：让 `close()` 直接发 RST 从而跳过 `TIME_WAIT`。这是**故意破坏协议保证**，只在测试对端健壮性或踢掉恶意连接时用，见 [[2.5 TCP连接关闭与异常状态#8. SO_LINGER|SO_LINGER]]。
...
参数细节和整条链路上的其他闸口见 [[8.2 TCP内核队列与参数调优|TCP 内核队列与参数调优]]。
...
- [[2.5 TCP连接关闭与异常状态]] - `close`/`shutdown`、半关闭、RST、`SO_LINGER`
- [[8.2 TCP内核队列与参数调优]] - 全链路闸口与端口耗尽排查
- [[3.2 套接字选项]] - `setsockopt`/`getsockopt` 用法与其他选项
- [[8.1 Nagle算法]] - 同一章的另一个默认行为陷阱
