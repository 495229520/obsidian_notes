...
这篇笔记补充 [[2.1 TCP协议基础#5. 四次挥手|TCP 四次挥手]] 和 [[2.6 TIME_WAIT状态]] 没有展开的工程细节。
...
## 3. close 和 shutdown 的区别

两个函数看着都能“关连接”，但它们动的根本不是同一个东西：

![[图片/SVG/3_8_8_11_2.svg|1231]]
...
| `TIME_WAIT` 多是谁的问题 | 主动关闭方进入，详见 [[2.6 TIME_WAIT状态]] |
...
- [[2.1 TCP协议基础#5. 四次挥手]] - 四次挥手的报文时序与状态转换
- [[2.6 TIME_WAIT状态]] - 主动关闭方进入 TIME_WAIT 的原因与优化
- [[6.10 非阻塞connect与健壮收发]] - 非阻塞收发中的错误处理与优雅关闭时机
- [[6.4 epoll模型]] - epoll 中关闭连接的工程细节
