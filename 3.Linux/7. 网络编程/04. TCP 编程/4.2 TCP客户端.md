...
1. 创建套接字
2. 连接到服务器（触发 [[2.1 TCP协议基础#4. 三次握手|三次握手]]）
3. 与服务器进行数据交换
4. 关闭连接
...
图中那条琥珀色横线还纠正了一个常见误解：**三次握手不是 `accept()` 做的**。`listen()` 之后内核就已经在替你握手了，握完的连接排进[[3.3 listen和accept函数#2. 两个队列|全连接队列]]；`accept()` 只是从队列里取一条出来配个 fd。所以队列里有货时 `accept` 立刻返回，队列空了才阻塞。
...
| 步骤 | 函数 | 说明 |
|:---:|:---|:---|
| 1 | [[3.1 套接字#2. socket 函数\|socket()]] | 创建套接字 |
| 2 | connect() | 连接到服务器 |
| 3 | write()/read() | 数据收发 |
| 4 | close() | 关闭连接 |
...
| 参数 | 说明 |
|:---|:---|
| `sockfd` | 由 [[3.1 套接字#2. socket 函数\|socket()]] 创建的套接字描述符 |
...
第二种是阻塞式 `connect` 最难受的地方：这两分钟里线程什么也做不了，而且这个超时**不受 `SO_SNDTIMEO` 控制**。要缩短它，要么调内核参数，要么用非阻塞 connect 配 `select`/`poll` 自己计时，见 [[6.10 非阻塞connect与健壮收发]]。
...
默认约 28000 个。朝**同一个目标 IP:Port** 发起的并发连接数就被这个宽度卡住，用尽后 `connect` 返回 `EADDRNOTAVAIL`；再叠加大量 TIME_WAIT 占着端口不放就更早撞线（见 [[2.6 TIME_WAIT状态]]）。压测机连不上更多连接时先查这个数，别急着怀疑服务端——算法见 [[1.3 端口与五元组#3.1 「端口只有 65535 个」到底限制了谁|7.3]]。
...
示例里 `read()` 只调了一次，够用是因为服务端只发了一行短消息。TCP 是**字节流**：一次 `read` 返回的字节数可能少于对端一次 `write` 发出的量，也可能一次就带回对端好几次 `write` 的内容。按「消息」处理数据时必须自己划边界，见 [[4.7 TCP的粘包和拆包]]。
...
- `socket()`：创建 [[3.1 套接字|套接字]]，用于建立网络通信的端点
- `bind()`：绑定 [[1.2 IP地址与CIDR|IP地址]] 和 [[1.3 端口与五元组|端口]]，服务端必须调用
- `listen()`：将套接字转为 [[3.3 listen和accept函数#3. listen 函数|监听]] 状态
- `accept()`：[[3.3 listen和accept函数#4. accept 函数|接受]] 客户端连接
- `connect()`：客户端连接服务器
...
2. **握手不是 accept 做的**：`listen()` 之后内核就自己握手了，`accept()` 只是从[[3.3 listen和accept函数#2. 两个队列|全连接队列]]取一条
...
- [[3.3 listen和accept函数#2. 两个队列]] - 半连接队列与全连接队列工作机制
- [[3.Linux/7. 网络编程/02. TCP 协议/2.2 TCP原理]] - TCP 通信原理概述
- [[4.1 TCP服务端]] - 服务端编程
- [[2.1 TCP协议基础]] - 三次握手、四次挥手详解
- [[3.1 套接字]] - Socket 函数详解
