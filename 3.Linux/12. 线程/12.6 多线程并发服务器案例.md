...
本节通过一个完整的案例，展示如何综合运用前面学习的知识构建一个**多线程并发 TCP 服务器**。这个案例整合了 [[3.1 套接字|Socket 编程]]、[[12.2 线程的创建与运行|线程创建]]、[[12.3 互斥量|互斥量]] 等核心概念。
...
- **IO 多路复用**：使用 select/poll/epoll（见 [[6.1 IO复用简介|IO 复用]]）
...
这也是为什么 [[6.4 epoll模型|epoll]] 和 [[12.7 条件变量与线程池|线程池]] 要连在一起学：前者解决“很多 fd 怎么等”，后者解决“耗时任务交给谁做”。
...
8. **设置 SO_REUSEADDR**：避免服务器重启时出现 "Address already in use" 错误（与 [[2.1 TCP协议基础#5.3 TIME_WAIT|TIME_WAIT 状态]] 相关）
