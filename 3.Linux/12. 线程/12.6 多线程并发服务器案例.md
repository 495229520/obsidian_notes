...
```text
client connections
-> epoll / async runtime 接收请求
-> CPU worker 做 tokenizer、参数校验、batch 组装
-> GPU worker 或调度线程发起 CUDA kernel / vLLM engine step
-> metrics 线程统计 TTFT / TPOT / TPS / 显存占用
```
...
6. **工程升级方向**：从“每连接一线程”升级到 `epoll + 线程池 + 有界队列`，再进一步理解 LLM 推理服务中的 CPU-GPU 调度和背压
