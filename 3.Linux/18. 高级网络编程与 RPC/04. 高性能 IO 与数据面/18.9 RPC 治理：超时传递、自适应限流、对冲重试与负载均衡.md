...
> `SO_RCVTIMEO`、`SO_SNDTIMEO`、TCP 重传计时器、HTTP/2 keepalive 和 RPC deadline 解决的是不同问题。[[2.1 TCP协议基础]]负责可靠字节流；RPC deadline 描述的是“这项业务结果何时失去价值”。仅设置 socket timeout 无法正确覆盖排队、LB、重试、服务端计算和回包的总耗时。UNP 对 socket timeout 的讨论可作为底层背景，但不能替代端到端 deadline。[^unp-timeout]
