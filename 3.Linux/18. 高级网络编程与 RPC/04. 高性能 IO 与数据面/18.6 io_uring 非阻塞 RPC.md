...
# 18.6 io_uring：用 SQE/CQE 推进非阻塞 RPC

[[18.1 RPC 协议模板]] 已经定义 14 B 帧头、`request_id`、`Add` 的请求/响应消息体和错误边界；[[18.4 C++ Coroutine 非阻塞 RPC]] 又把“等待响应”包装为 `co_await`。本篇继续保留同一 RPC 协议，只把 socket I/O 的驱动方式换成 `io_uring`。
...
- [[18.1 RPC 协议模板]]；[[18.4 C++ Coroutine 非阻塞 RPC]]。
