...
# 18.7 RDMA Verbs 与 CQ：用 WR/WC 承载非阻塞 RPC

[[18.1 RPC 协议模板]] 定义的 `Frame`、`request_id`、方法号和状态码与 TCP 无关；它们同样可以放进 RDMA 消息。本篇先采用最容易与原案例对应的 **RC QP + SEND/RECV**：一条 SEND 携带一条完整 RPC Frame，接收端预贴 RECV，CQ 交付 Work Completion（WC）。
...
[[18.4 C++ Coroutine 非阻塞 RPC]] 中的 awaiter 不需要认识 verbs：
...
- [[18.1 RPC 协议模板]]；[[18.4 C++ Coroutine 非阻塞 RPC]]；[[18.6 io_uring 非阻塞 RPC]]。
