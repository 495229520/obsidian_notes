...
sequenceDiagram
    ...
    Client->>EL: CallAsync(method=Add, body=8B)
    EL->>EL: 登记 pending_calls[id]
    EL-->>App: 立即返回

    Note over EL: epoll_wait 中等待任一连接事件
    Peer-->>EL: Response(id, OK, body=4B)
    EL->>EL: ReadU32(body) = 7
    EL-->>App: callback(AddResult{7})
```
...
sequenceDiagram
    ...
    EL->>EP: epoll_wait(...)
    activate EL
    Note over EL: 无事件时统一睡眠
    Sock-->>EP: fd 状态变化
    EP-->>EL: EPOLLIN / EPOLLOUT / RDHUP
    EL->>Conn: HandleEvents(events)
    deactivate EL
