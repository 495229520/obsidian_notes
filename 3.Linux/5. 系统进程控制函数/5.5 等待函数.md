...
> 单个场景直达：[status 位布局](<../../图片/HTML/3.Linux/5. 系统进程控制函数/5.1-5.5 exec·退出·status·进程组动画.html#scene=status>) · [退出出口](<../../图片/HTML/3.Linux/5. 系统进程控制函数/5.1-5.5 exec·退出·status·进程组动画.html#scene=exit>)

## 简介

[[5.1 系统进程控制函数#1. fork() - 创建子进程|fork()]] 之后父子两条路各跑各的，子进程先结束是常态。但子进程 `exit()` 并不等于它从系统里消失了：内核会把它的退出状态扣下来，一直留到父进程主动来取。取的动作就是 `wait()` 和 `waitpid()`。
...
`system()` 多出来的那层 `sh -c` 会解释 `;`、`|`、`` ` `` 这些元字符，命令字符串里拼进用户输入就有注入风险。这一部分连同 `fork` + `exec` 的替代写法在 [[5.1 系统进程控制函数#4. system() - 执行 Shell 命令|5.1]] 里讲过，这里不重复。
...
|--------|------|
| `> 0` | 等待这一个指定 PID 的子进程 |
| `= 0` | 等待与调用者同[[5.4 进程组识别码|进程组]]的任意子进程 |
| `= -1` | 等待任意子进程，和 `wait()` 一样 |
