...
## 2. sigsetjmp() 和 siglongjmp() - 掩码要不要一起存

### 掩码是怎么被锁住的

![[图片/SVG/3_5_5_3_3.svg|1307]]
...
handler 里也别用 `printf`。它不是异步信号安全的函数，主流程正在 `printf` 中途被打断时进来会踩坏 `stdio` 的内部状态。要输出就用 `write()`，完整的约束见 [[11.7 僵尸进程#4.7 handler 里的三条约束|handler 里的三条约束]]。
