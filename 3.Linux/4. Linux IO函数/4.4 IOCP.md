...
> [!tip] 交互动画
> 本篇核心难点的过程可视化（WSARecv 投递与 WSA_IO_PENDING 语义 · CompletionKey/OVERLAPPED 两层上下文 · worker 取包与补投 · GQCS 失败完成四分支与退出包）：
> [▶ 在浏览器中打开动画](<../../图片/HTML/3.Linux/4. Linux IO函数/4.4.1 IOCP动画.html>)　·　更多见 [[网络编程·动画合集]]
> 也可直达单个场景：[请求投递](<../../图片/HTML/3.Linux/4. Linux IO函数/4.4.1 IOCP动画.html#scene=post>) · [两层上下文](<../../图片/HTML/3.Linux/4. Linux IO函数/4.4.1 IOCP动画.html#scene=ctx>) · [worker 取包](<../../图片/HTML/3.Linux/4. Linux IO函数/4.4.1 IOCP动画.html#scene=loop>) · [失败完成](<../../图片/HTML/3.Linux/4. Linux IO函数/4.4.1 IOCP动画.html#scene=fail>)
...
![[图片/3.Linux/4. Linux IO函数/4_4_IOCP_two_layer_context.svg|1051]]
...
![[图片/3.Linux/4. Linux IO函数/4_4_IOCP_single_connection_lifecycle.svg|1048]]
...
### 10.1 错误 1：把 `WSA_IO_PENDING` 当成失败
...
### 10.2 错误 2：多个未完成 I/O 复用同一个 `OVERLAPPED`
...
### 10.4 错误 4：`GetQueuedCompletionStatus` 返回 `FALSE` 就以为“IOCP 坏了”
...
### 10.5 错误 5：在 `closesocket()` 后立刻释放所有 I/O 相关资源
...
### 10.6 错误 6：忘记 `SO_UPDATE_ACCEPT_CONTEXT`
...
### 10.7 错误 7：在 overlapped 模式下还依赖 `lpNumberOfBytesRecvd/lpNumberOfBytesSent`
