...
# 7.4.2 gRPC：用 `.proto` 定义服务并生成 RPC 代码
...
协议编解码相关的现代 C++ 基础（如 `std::byte`、`std::span`、`using`、`constexpr`）见 [[15.13 std::span|15.13 std::span 连续内存视图]]。
...
```text
先理解 7.4.1 的手写二进制 RPC 协议
        ↓
再理解 7.4.2 的 gRPC 如何把这些机制标准化并生成代码
（底层内存视图与零拷贝机制可深入阅读 [[15.13 std::span|15.13 std::span]]）
```
