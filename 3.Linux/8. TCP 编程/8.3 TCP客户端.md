...
1. 创建套接字
2. 连接到服务器（触发 [[7.5 TCP协议基础#三次握手|三次握手]]）
3. 与服务器进行数据交换
4. 关闭连接
...
默认约 28000 个。朝**同一个目标 IP:Port** 发起的并发连接数就被这个宽度卡住，用尽后 `connect` 返回 `EADDRNOTAVAIL`；再叠加大量 TIME_WAIT 占着端口不放就更早撞线（见 [[10.2 TIME_WAIT状态]]）。压测机连不上更多连接时先查这个数，别急着怀疑服务端——算法见 [[7.3 端口#「端口只有 65535 个」到底限制了谁|7.3]]。
