...
1. 创建套接字
2. 连接到服务器（触发 [[7.5 TCP协议基础#4. 三次握手|三次握手]]）
3. 与服务器进行数据交换
4. 关闭连接
...
| 步骤 | 函数 | 说明 |
|:---:|:---|:---|
| 1 | [[7.6 套接字#2. socket 函数\|socket()]] | 创建套接字 |
| 2 | connect() | 连接到服务器 |
| 3 | write()/read() | 数据收发 |
| 4 | close() | 关闭连接 |
...
| 参数 | 说明 |
|:---|:---|
| `sockfd` | 由 [[7.6 套接字#2. socket 函数\|socket()]] 创建的套接字描述符 |
...
- `listen()`：将套接字转为 [[7.7 listen和accept函数#3. listen 函数|监听]] 状态
- `accept()`：[[7.7 listen和accept函数#4. accept 函数|接受]] 客户端连接
- `connect()`：客户端连接服务器
