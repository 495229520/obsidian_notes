...
> [!success] 当前基线：v0.3
> 2026-08-18 最新规划已经确认：**下位机统一管理和裁决全部控制权**。数据中台只接单、查询执行摘要、消费实时消息、展示统一视图、调用下位机控制/调试接口并做权限和审计。旧的“中台依据调度业务状态协调生产/调试互斥”已废弃。

## 1. 活跃文档

| 文档 | 负责回答 | 状态 |
| --- | --- | --- |
| [[MFMS新中台-架构构造思路]] | 当前系统边界、组件、数据与最高原则 | **v0.3 当前架构入口** |
| [[MFMS新中台-逐层设计工作台]] | 控制、Stream、订单、恢复、审计与实施切片 | **v0.3 唯一详细工作台** |
| [[MFMS新中台-待确定问题清单]] | P0 接口与业务问题 | **唯一未决项登记簿** |
| [[MFMS新中台-Trellis工程结构方案]] | v0.3 如何落入 ARCHMAP/spec/task | **当前项目落位方案** |
| [[MFMS新中台-架构图.drawio]] | 系统上下文、控制权、数据权威 | **v0.3 可编辑图源** |
| [[14.复合mfms/14.1 Task/Trellis Task 总览\|Trellis Task 总览]] | 项目任务状态展示 | 自动生成、只读 |

一个主题只能有一份活跃正文。若上表与旧材料冲突，以上表中的 v0.3 文档为准。
...
| 订单 | 中台拥有 order_request；调度拥有 execution 与 binding；推荐变更请求分表 |
| 多调度实例 | 推荐用 order_uid 唯一约束竞争首次接管；中台不做选主 |
| Adapter | 内部 ADK 编排、控制策略和恢复移出中台范围 |
| 调度连接 | 基线不需要 gRPC 直连；订单经 MySQL，实时消息经 Stream |
...
1. 已确认事实优先于推荐方案；
2. v0.3 优先于 v0.2 和 2026-07 历史拍板；
...
| 材料 | 位置 | 用途 |
| --- | --- | --- |
| v0.3 最新规划 | 2026-08-18《MFMS 数据中台架构方案 v0.3：下位机统一管理控制权》；摘要持久化在项目 architecture task 的 research 中 | 本轮最高优先输入 |
