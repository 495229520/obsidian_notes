...
| 章 | 主题 | 回答的核心问题 | 状态 |
| --- | --- | --- | --- |
| 0 | 数据路径硬件基础 | Cache/TLB/DMA/PCIe/NUMA 与 Benchmark 方法 | 骨架（未写） |
| 1 | Linux IO 路径 | 一次 read/write 到底经过什么？缓存何时命中、数据何时真正落盘？ | 正文 + 补缺骨架 |
| 2 | io_uring | 怎样把系统调用开销摊薄？异步 IO 与 epoll 的本质区别？ | 正文 + 补缺骨架 |
| 3 | NVMe 与 SSD | 设备端如何并行执行命令？SSD 为什么会"越用越慢"？ | 正文 + 补缺骨架 |
| 4 | RDMA 与 RoCE | 网卡如何绕过内核直接读写内存？无损网络靠什么保证？ | 正文 + 补缺骨架 |
| 5 | NVMe-oF 与 SPDK | 本地盘如何远端化？内核态与用户态路径差在哪？ | 正文 + 补缺骨架 |
| 6 | 存储引擎 | WAL/LSM 如何在"慢介质 + 会崩溃"的前提下保证不丢数据？ | 正文 + 补缺骨架 |
| 7 | 分布式存储 | 一致性/副本/故障后为何仍然正确？ | 正文 + 原理骨架 |
| 8 | AI GPU 数据路径 | 数据怎样最快到达显存？KV Cache 分层与 GDS 回退 | 正文 + 补缺骨架 |
| 9 | 性能观测与可靠性 | 贯穿全程的工具与 p99/SLO/故障注入 | 正文（贯穿使用） |
| 10 | 控制面（选修） | 编排、拓扑调度、故障摘除、自动 Benchmark | 正文（选修） |
...
在 Obsidian 中可用标签 `未写` 或属性 `status: 未写` 筛选待写清单。写完后删掉 `未写` 标签并把 `status` 改为 `已写`。
