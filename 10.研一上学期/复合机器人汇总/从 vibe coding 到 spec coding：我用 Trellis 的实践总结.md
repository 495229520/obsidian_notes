---
title: 从 vibe coding 到 spec coding：我用 Trellis 的实践总结
source:
author:
...
> 但最麻烦的地方，我觉得在于它每次改的时候，其实更多是在解决眼前这个 局部问题。
...
但是如果一个项目长期都靠这种方式一直去堆代码的话，每一次修改都可能只是 局部最优解法。单次看是没什么问题，但是几轮之后呢？ 代码里的会有大量的重复逻辑特殊判断，而且风格漂移，隐藏bug也会越来越多。
...
[![Image 1|1074](https://cdn3.ldstatic.com/optimized/4X/d/a/1/da1bd18563d7e4141ce5ad6fbb1772120a71fac7_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/d/a/1/da1bd18563d7e4141ce5ad6fbb1772120a71fac7.jpeg)
...
[![Image 3](https://cdn3.ldstatic.com/optimized/4X/4/a/b/4abc1582e55b2a76b52310aeaba083190749e060_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/4/a/b/4abc1582e55b2a76b52310aeaba083190749e060.jpeg)
...
[![Image 4](https://cdn3.ldstatic.com/optimized/4X/9/3/e/93ea62e9eb4b0a8bb70e61c5b338f2e9d30c43fa_2_425x500.png)](https://cdn3.ldstatic.com/original/4X/9/3/e/93ea62e9eb4b0a8bb70e61c5b338f2e9d30c43fa.png)

[![Image 5](https://cdn3.ldstatic.com/optimized/4X/b/b/1/bb13ceb89f99274cfcc91782d011ee74580a3c8d_2_520x500.png)](https://cdn3.ldstatic.com/original/4X/b/b/1/bb13ceb89f99274cfcc91782d011ee74580a3c8d.png)

[![Image 6](https://cdn3.ldstatic.com/optimized/4X/0/e/5/0e54331fa3977278bd3d74d6179fe6e5b2277811_2_690x263.png)](https://cdn3.ldstatic.com/original/4X/0/e/5/0e54331fa3977278bd3d74d6179fe6e5b2277811.png)
...
[![Image 7](https://cdn3.ldstatic.com/optimized/4X/c/c/f/ccf62208ae9867c4b9d4af1b5bd9181d8ce8d87b_2_690x330.png)](https://cdn3.ldstatic.com/original/4X/c/c/f/ccf62208ae9867c4b9d4af1b5bd9181d8ce8d87b.png)
...
> 因为它主要靠仓库根目录的 `AGENTS.md` 做 prelude，再通过 `UserPromptSubmit` hook 注入 workflow-state。新版本 Codex CLI 里 hooks 可能已经默认开启，但如果 `/` 菜单里看不到 Trellis 的 command 或 skill，或者 workflow 指引没有自动注入，可以检查 `~/.codex/config.toml` 里是否开启了 `features.hooks = true`，并通过 `/hooks` 审批 Trellis 安装的 hook。即便 hook 没有运行，Trellis 仍然可以通过 `AGENTS.md` fallback 让 AI 读取 `trellis-start`，只是体验会弱一些。
...
如果 AI 不知道当前处于哪个阶段，就很容易跳步骤。需求还没确认就开始写代码，代码写完以后又忘了检查，检查完以后也不沉淀经验。

所以 Trellis 是用 workflow-state 来解决这个问题的。

在支持 hook 的平台上，每条用户 prompt 进来时，Trellis 都会给 AI 注入当前任务状态。常见的实时状态主要包括当前开发者、活跃任务、任务阶段，以及下一步应该执行的动作。

所以这一点很关键。Trellis 除了告诉 AI 项目规则，还会告诉 AI 当前走到了哪一步。有了这个状态，AI 每一轮都更容易判断现在应该继续澄清需求，还是开始实现，还是进入检查，还是做收尾。

* * *

### 第三步：先判断当前 turn 要不要建 task

很多人一听 spec coding，就会担心所有事情都要建任务、写 PRD、走流程。就好比我们现在用 superpowers skill 进行开发时，很多人觉得它太重，不管什么都要全部走一遍，既浪费时间又增加成本，甚至想直接卸载。

而 Trellis 的实际思路没有像 superpowers 这么重。

当我们发起一个需求时，Trellis 会先判断当前 turn 的性质。

如果只是问一个技术问题，比如某个库怎么用、某个报错是什么意思，它可以直接回答，不需要建 task。

如果只是改一个很小的 bug，当前对话就能说清楚，也可以走 inline。它最多提醒我们要不要创建 task，我们说不用，就直接处理。

只有当任务涉及多文件、多模块，需要设计或需要复盘时，Trellis 才会建议创建 task。

这也是我觉得它比 superpowers 更适合日常开发的地方。它不会把所有事情都流程化，而是根据任务复杂度决定要不要进入完整任务流程。
...
需求进入 planning 以后，Trellis 做的事情就像是把一句临时需求整理成可以持续推进的任务资产。

我们平时 vibe coding 时可能会直接说：“帮我做一个登录功能。”然后 AI 直接开始写，短期看起来很快。但很多关键问题会被跳过，比如登录失败要怎么处理、token 怎么设置过期、是否支持第三方登录、老用户数据怎么兼容等。

Trellis 会在 planning 阶段先把这些问题问清楚，再写进 task 目录。常见产物包括：

> [!example] Task artifacts
> - `prd.md`：说明需求目标、范围、边界和验收标准
> - `design.md`：记录技术方案、模块关系与关键取舍
> - `implement.md`：拆分实现步骤和执行清单
> - `research.md`：保存调研结果、参考资料与待验证结论
...
planning 完成后，任务就会进入 in_progress。这时 AI 开始实现，但它不会只根据我们最后一句 prompt 写代码。它会综合读取当前任务状态、task artifacts、相关 spec、调研材料和仓库里的实际代码。
...
这里的 `spec/` 也不会一次性全部塞给 AI。默认模板主要包括 `backend/`、`frontend/`、`guides/`，AI 会按任务需要选择相关规范。这样比一个超长 rules 文件更容易维护，也更不容易造成上下文过载。

* * *

### 第六步：check 会对照规范复核

很多 AI 编码流程里的检查，最后会变成一句“跑一下 lint 和 test”。这样做有用，但还不够，因为有些问题测试抓不到。比如接口错误格式不符合项目约定、权限校验漏了、目录结构不符合团队习惯，或者某个模块的依赖方向反了。

Trellis 的 check 阶段更接近一次小型 code review。它会查看当前 diff，也会读取 `check.jsonl` 里声明的 spec 和 research，然后对照规范检查代码。

所以在我看来，Trellis check 的价值不只是多跑几条命令。它会把检查标准从临时感觉，变成任务和规范里明确写下来的内容。

* * *

### 第七步：update-spec 只沉淀稳定经验

Trellis 在任务做完以后，还有一个关键动作，就是判断有没有经验值得沉淀。

这一步对应的是 `trellis-update-spec`。

这里容易有一个误解，好像 AI 做完任务后，会把所有经验都自动写回 spec。实际不应该这样做。spec 如果什么都收，最后就会变成垃圾桶，越写越长、越写越乱，AI 反而更难抓住重点。

更合理的做法是任务收尾时，让 AI 引导我们判断哪些经验值得长期复用。例如：

> [!example] 适合写入 spec 的内容
> - 某类接口必须统一返回错误格式
> - 某个模块不能直接依赖另一个模块
...
但如果只是某个功能的登录接口改成 `/api/v2/login`，这类信息更适合留在 task。

所以 update-spec 的重点在于筛选，避免把 spec 变成堆积材料。
...
[![Image 11|1316](https://cdn3.ldstatic.com/optimized/4X/d/6/0/d60c229f9810141d26541e283c7ed7dd6acd3412_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/d/6/0/d60c229f9810141d26541e283c7ed7dd6acd3412.jpeg)
...
[![Image 12](https://cdn3.ldstatic.com/optimized/4X/1/8/f/18f6b3bb665871b456ff9d615ae689e0607a9789_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/1/8/f/18f6b3bb665871b456ff9d615ae689e0607a9789.jpeg)
...
[![Image 13](https://cdn3.ldstatic.com/optimized/4X/1/1/8/11826308e9a6a2cca7488c3f295d5dc934157355_2_690x376.jpeg)](https://cdn3.ldstatic.com/original/4X/1/1/8/11826308e9a6a2cca7488c3f295d5dc934157355.jpeg)
...
[![Image 15](https://cdn3.ldstatic.com/optimized/4X/4/1/c/41caf8856082be393b63aaf82ed6287d40bc6093_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/4/1/c/41caf8856082be393b63aaf82ed6287d40bc6093.jpeg)
...
[![Image 16](https://cdn3.ldstatic.com/optimized/4X/6/9/a/69a76f280e82fdd767de6f1b7f0e72108ce128f0_2_690x385.jpeg)](https://cdn3.ldstatic.com/original/4X/6/9/a/69a76f280e82fdd767de6f1b7f0e72108ce128f0.jpeg)
