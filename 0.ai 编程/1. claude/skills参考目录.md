...
| `english-drill` | "今日任务"、"批改作文"、"练口语"、"精听"、"复盘" | 六级 + 雅思备考日常驱动：排每日任务、批改作文、口语反馈、精听复盘 |

两个导师型 Skill，留下的理由都是内嵌了模型推不出来的数据。

`numerical-analysis` 内嵌 2020–2024 五套期末真题的考频表，任何"这个考不考"的判断以该表为准。对应笔记在 `7.LLM应用/5.数值分析/` 和 `7.LLM应用/6.数值分析笔记/`。

`english-drill` 内嵌学习者基线（CET-4 444 分、部分词不会读）、四个阶段的日期锚点、剑桥雅思 15 本音频各不相同的命名规则、词表 JSONL 的字段结构。对应笔记在 `13. 英文/`，主方案 [[00.学习方案]]，资源清单 [[01.资源盘点]]。
