...
这里的 **Graph** 是“任务依赖图”，不是“让任务自动并行”的意思。它把一串 CUDA 操作记录成节点和依赖关系，例如：

```text
H2D copy -> kernel -> D2H copy
```

这张图表达的是：kernel 依赖 H2D 完成，D2H 又依赖 kernel 完成。Graph 的价值是把这串固定任务提前记录和实例化，后面重复执行时，CPU 不用每次都逐个提交 copy 和 kernel。

> [!important] CUDA Graph 不会打破 stream 顺序
> 如果 100 次 `cudaGraphLaunch` 都提交到同一个 stream，那么 GPU 侧仍然按 stream 顺序执行。CUDA Graph 减少的是 CPU 提交任务的开销，不是把同一个 stream 里的任务自动变成并行。
...
使用 CUDA Graph 后，先把这三个操作捕获成一张图：

```text
Graph = H2D -> kernel -> D2H
```
...
这时 CPU 提交的是 `100` 次 graph launch。每次 graph launch 都代表一整串固定任务，而不是分别提交 H2D、kernel、D2H。

所以这段代码虽然使用的是同一个 `stream`，但它仍然有意义：

```text
CPU 侧：更少的 Runtime / Driver 提交开销
GPU 侧：同一个 stream 内仍然按顺序执行
```

一句话记：**CUDA Graph 解决的是“重复提交固定任务序列”的开销，不是解决 stream 内部并行问题。**

### 7.2 CUDA Graph 和 overlap 的关系

CUDA Graph 本身不像 `cudaMemcpyAsync + 多 stream + pinned memory` 那样，重点在制造流水线 overlap。它主要解决的是 **CPU 反复提交固定任务序列的开销**。

两者的区别可以这样看：

| 机制 | 主要解决什么 | 是否强调流水线 overlap |
|---|---|---|
| `cudaMemcpyAsync + 多 stream` | 让不同 chunk 的 H2D / kernel / D2H 有机会重叠 | 是 |
| `CUDA Graph` | 把固定 CUDA 任务序列打包复用，减少 CPU 提交开销 | 不是重点 |
...
并且每次都 launch 到同一个 stream，那么 GPU 侧仍然会顺序执行：

```text
Graph 0: H2D -> kernel -> D2H
Graph 1: H2D -> kernel -> D2H
Graph 2: H2D -> kernel -> D2H
...
```
...
```text
Graph 0 的 kernel 和 Graph 1 的 H2D 重叠
```

但是，Graph 可以捕获一个已经设计好的 overlap 结构。例如你先用多 stream 写出：

```text
stream0: H2D c0 -> kernel c0 -> D2H c0
stream1: H2D c1 -> kernel c1 -> D2H c1
stream2: H2D c2 -> kernel c2 -> D2H c2
```

如果这些任务和依赖关系被捕获进 Graph，那么这个 Graph 就包含了多 stream pipeline 的结构。此时 Graph 只是把这个结构打包复用，真正产生 overlap 的仍然是多 stream、异步拷贝、pinned memory 和硬件 copy engine。

一句话记：**`cudaMemcpyAsync + 多 stream` 是为了制造 overlap；CUDA Graph 是为了复用已有任务结构、减少提交开销。Graph 可以包含 overlap，但 Graph 本身不等于 overlap。**
