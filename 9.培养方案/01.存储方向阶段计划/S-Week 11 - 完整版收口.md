...
- 八股（1 章）：文件系统 + 串讲收口。文件系统过 inode / 目录项 / journaling 概念（OSTEP Crash Consistency 章 + [[4.1 打开、读取、写入、关闭]]、[[4.2 重定向、同步]]），mmap 复面用 [[S-Week 4 - mmap 与读路径对比]] 的实验数据回答。网络收口：把三次握手 → 状态机 → 内核队列 → 拥塞控制 → epoll → io_uring 串成 10 分钟脱稿故事线（[[6.5 串讲]] 的方法扩到全网络栈）。
