...
| 操作类别 | `unordered_map<K, V>` | `unordered_set<K>` | 平均时间复杂度 | 说明与注意事项 |
| :--- | :--- | :--- | :---: | :--- |
| **存在性检查** | `mp.count(key)`<br>`mp.contains(key)` *(C++20)* | `set.count(x)`<br>`set.contains(x)` *(C++20)* | $O(1)$ | 存在返回 1 (true)，不存在返回 0 (false)；推荐代替 `[]` 查询 |
| **查找迭代器** | `auto it = mp.find(key);`<br>`if (it != mp.end()) ...` | `auto it = set.find(x);`<br>`if (it != set.end()) ...` | $O(1)$ | 找到返回元素迭代器，未找到返回 `.end()`；只读查询不插新键 |
| **插入/写入** | `mp[key] = val;`<br>`mp.insert({key, val});`<br>`mp.emplace(key, val);` | `set.insert(x);`<br>`set.emplace(x);` | $O(1)$ | `[]` 不存在则插入默认值；`insert`/`emplace` 若已存在则**不覆盖** |
| **条件插入** | `mp.try_emplace(key, val);` *(C++17)* | — | $O(1)$ | 仅当 key 不存在时才构造并插入，避免无谓临时对象构造 |
| **频次计数** | `++mp[key];` | — | $O(1)$ | key 不存在时先自动插入 `key -> 0` 再递增为 1；计数最简写法 |
| **带检查读取** | `mp.at(key);` | — | $O(1)$ | 若 key 不存在会抛出 `std::out_of_range` 异常，不插入新元素 |
| **按键删除** | `mp.erase(key);` | `set.erase(x);` | $O(1)$ | 删除指定 key，返回删除成功的元素个数（0 或 1） |
| **按迭代器删除** | `it = mp.erase(it);` | `it = set.erase(it);` | $O(1)$ | 传入迭代器删除，返回被删元素下一个位置的有效迭代器 |
| **清空容器** | `mp.clear();` | `set.clear();` | $O(n)$ | 清空所有元素，容器桶数不一定缩小 |
| **容量/判空** | `mp.size()` / `mp.empty()` | `set.size()` / `set.empty()` | $O(1)$ | 获取当前元素数量 / 判断是否为空 |
| **预分配空间** | `mp.reserve(n);` | `set.reserve(n);` | $O(n)$ | 提前预留 $n$ 个元素的桶空间，避免频繁扩容导致 rehash |
...
if (auto it = mp.find(key); it != mp.end()) {
    int val = it->second; // 找到对应 value
}

// 2. 遍历 map（C++17 结构化绑定）
for (const auto& [k, v] : mp) {
    // k 为只读键 (const K), v 为值引用或副本 (V)
}
...
> [!tip] 核心避坑口诀
> - **查存在用 `count` / `find`，勿用 `[]`**：`mp[key]` 在 key 不存在时会默认构造并写入（如 `int` 插入 0），不仅产生副作用还会改变容器大小。
> - **先查后插防自用**：在两数之和/配对问题中，必须“先查询配对值 `target - nums[i]`，后把当前元素插入哈希表”，防止同一元素与自身配对。
> - **大规模插入先 `reserve`**：已知数据规模时，提前 `mp.reserve(n)` 可以大幅减少扩容重建（rehash）的常数开销与迭代器失效风险。
...
![[图片/SVG/5_2_8_1.svg|1390]]

图中左侧表示线性查找：每次查询都可能从头扫到尾；右侧表示哈希查找：先通过 `hash(key)` 定位到 bucket，再在 bucket 内检查元素。下部的 bucket 链说明了哈希冲突：多个 key 落到同一个 bucket 时，bucket 内部仍然需要继续比较。
...
`load_factor` 表示元素数量和桶数量的比例，装得太满就扩容重哈希：

![[图片/SVG/5_2_8_2.svg|1390]]

rehash 本身是一次 `O(n)` 的整表重建（所有 key 重新算桶）；但扩容采用「桶数翻倍」，均摊到每次插入仍是平均 `O(1)`。桶数翻倍后，`hash(key) % bucket_count` 的除数变了，很多 key 会落到不同的桶，链随之变短。
...
| --------------------------- | -------------- | ------------------------ | --------- | --------- |
| 1. 两数之和                     | 哈希查找           | 扫描时查找 `target - nums[i]` | hot100    | 302       |
| 41. 缺失的第一个正数                | 原地哈希           | 把数字放到对应下标位置              | hot100    | 113       |
| 560. 和为 K 的子数组              | 前缀和 + 哈希计数     | 查找历史前缀和 `prefix - k`     | hot100    | 78        |
| 442. 数组中重复的数据               | 原地哈希 / 标记      | 利用数值和下标映射找重复元素           | hot300    | 44        |
| 剑指 Offer 50. 第一个只出现一次的字符    | 哈希计数           | 先统计频次再按原序找第一个唯一字符        | 低频        | 11        |
| 287. 寻找重复数                  | 原地映射 / 快慢指针    | 可看作值到下标的环                | hot300    | 36        |
| 128. 最长连续序列                 | 哈希集合           | 只从序列起点开始扩展               | hot100    | 92        |
| 1004. 最大连续 1 的个数 III        | 滑动窗口计数         | 窗口内最多允许 `k` 个 0          | hot300    | 28        |
| 395. 至少有 K 个重复字符的最长子串       | 分治 / 计数        | 按出现次数不足 `k` 的字符切分        | hot300    | 27        |
| 136. 只出现一次的数字               | 异或 / 计数        | 异或抵消重复数字                 | hot300    | 53        |
| 316. 去除重复字母                 | 单调栈 + 计数       | 保证字典序最小且每个字符出现一次         | hot300    | 25        |
| 692. 前 K 个高频单词              | 哈希计数 + 堆/排序    | 统计词频后按频率和字典序排序           | hot300    | 24        |
| 18. 四数之和                    | 排序 + 双指针 + 去重  | 固定两个数后做两数和               | hot300    | 22        |
| 剑指 Offer 39. 数组中出现次数超过一半的数字 | 哈希计数 / 投票      | 统计频次或 Boyer-Moore        | hot300    | 24        |
| 459. 重复的子字符串                | 字符串周期          | 判断是否由某个子串重复构成            | hot300    | 21        |
| 260. 只出现一次的数字 III           | 位运算分组          | 用最低位 1 把两个唯一数分组          | hot300    | 20        |
| 剑指 Offer 03. 数组中重复的数字       | 原地哈希           | 把数字交换到对应下标               | hot300    | 21        |
| 242. 有效的字母异位词               | 字符计数           | 比较两个字符串字符频次              | hot300    | 18        |
| 80. 删除排序数组中的重复项 II          | 计数 / 原地去重      | 每个值最多保留两次                | hot300    | 18        |
| 49. 字母异位词分组                 | 哈希分组           | 排序字符串或频次数组作为键            | hot300    | 17        |
| 706. 设计哈希映射                 | 哈希表            | 自实现键值映射                  | hot300    | 15        |
| 204. 计数质数                   | 素数筛            | 埃氏筛统计小于 n 的质数            | hot300    | 15        |
| 532. 数组中的 K-diff 数对         | 哈希计数           | 按 k=0 和 k>0 分情况计数        | 低频        | -         |
| 697. 数组的度                   | 哈希记录           | 记录频次、首次和末次位置             | 低频        | 4         |
| 268. 丢失数字                   | 哈希 / 数学 / 异或   | 找出 `0..n` 中缺失的数          | hot300    | 16        |
| 540. 有序数组中的单一元素             | 二分 / 异或        | 成对元素下标规律                 | hot300    | 15        |
| 961. 重复 N 次的元素              | 哈希集合           | 找第一个重复出现元素               | 低频        | -         |
| 509. 斐波那契数                  | 记忆化 / DP       | 基础递推                     | hot300    | 21        |
| 438. 找到字符串中所有字母异位词          | 频次窗口           | 字符计数匹配                   | hot300    | 15        |
| 169. 多数元素                   | Boyer-Moore 投票 | 候选值与计数抵消，最终剩余多数元素        | hot100    | 68        |
## 相关链接

- [[算法·动画合集]] — 拉链冲突 + rehash / 两数之和查补数 / 最长连续序列 / 异位词分组 的交互动画
